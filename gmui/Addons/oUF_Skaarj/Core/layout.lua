    ...
    player = function(self, ...)
        ...
        if cfg.exp_rep.enable then 
            if UnitLevel('player') < MAX_PLAYER_LEVEL then		    
                local e = createStatusbar(self, cfg.texture, nil, cfg.treat.height, cfg.treat.width, 0, .7, 1, 1)
				e:SetFrameStrata('LOW')
				if cfg.exp_rep.unlock then
				    e:SetPoint(unpack(cfg.exp_rep.pos))
				...
				if cfg.exp_rep.unlock then
				    r:SetPoint(unpack(cfg.exp_rep.pos))
...
oUF:Factory(function(self)
    ...
    if cfg.uf.party then
		...
		if cfg.options.showRaid then
            ...
            party:SetPoint('RIGHT',cfg.unit_positions.Party.a,'LEFT',cfg.unit_positions.Party.x,cfg.unit_positions.Party.y)
	...
	if cfg.uf.tank then
	    ...
	    maintank:SetPoint('TOPLEFT', cfg.unit_positions.Tank.a, 'TOPLEFT', cfg.unit_positions.Tank.x, cfg.unit_positions.Tank.y)
